...
if (!function_exists('dwt_listing_submit_new_listing')) {
    ...
    function dwt_listing_submit_new_listing() {
        ...
        //$ad_desc = sanitize_textarea_field( $params['listing_desc'] );
        ...
        //$packg_id = '';
        //to compatible previous functionalty with expiry
        /* if (get_user_meta(get_current_user_id(), 'd_user_package_id', true) != '')
          {
          $packg_id = get_user_meta(get_current_user_id(), 'd_user_package_id', true);
          if(get_user_meta(get_current_user_id(), 'is_active_expiry', true))
          {
          if(get_post_meta($packg_id, 'listing_expiry', true) != '' && get_post_meta($packg_id, 'listing_expiry', true) != '-1')
          {
          update_post_meta($listing_id, 'dwt_no_expiry', 'yes');
          }
          else
          {
          update_post_meta($listing_id, 'dwt_no_expiry', 'no');
          }
          }
          else
          {
